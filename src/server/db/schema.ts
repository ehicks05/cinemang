...
export const showStatus = pgEnum('ShowStatus', [
	'ENDED',
	'CANCELED',
	'RETURNING_SERIES',
	'IN_PRODUCTION',
...
export const movie = pgTable(
	...
	(table) => [
		index('movie_language_id_idx').using(
			'btree',
			table.languageId.asc().nullsLast().op('text_ops'),
		),
		index('movie_released_at_idx').using(
			'btree',
			table.releasedAt.asc().nullsLast().op('date_ops'),
		),
		index('movie_title_idx').using(
			'gin',
			table.title.asc().nullsLast().op('gin_trgm_ops'),
		),
		index('movie_vote_average_idx').using(
			'btree',
			table.voteAverage.asc().nullsLast().op('float8_ops'),
		),
		index('movie_vote_count_idx').using(
			'btree',
			table.voteCount.asc().nullsLast().op('int4_ops'),
		),
	],
);
...
export const credit = pgTable(
	...
	(table) => [
		index('credit_movie_id_idx').using(
			'btree',
			table.movieId.asc().nullsLast().op('int4_ops'),
		),
		index('credit_person_id_idx').using(
			'btree',
			table.personId.asc().nullsLast().op('int4_ops'),
		),
		index('credit_show_id_idx').using(
			'btree',
			table.showId.asc().nullsLast().op('int4_ops'),
		),
		foreignKey({
...
export const mediaProvider = pgTable(
	'media_provider',
	{
	...
	(table) => [
		index('media_provider_movie_id_idx').using(
			'btree',
			table.movieId.asc().nullsLast().op('int4_ops'),
		),
		uniqueIndex('media_provider_movie_id_provider_id_key').using(
			'btree',
			table.movieId.asc().nullsLast().op('int4_ops'),
			table.providerId.asc().nullsLast().op('int4_ops'),
		),
		index('media_provider_provider_id_idx').using(
			'btree',
			table.providerId.asc().nullsLast().op('int4_ops'),
		),
		index('media_provider_show_id_idx').using(
			'btree',
			table.showId.asc().nullsLast().op('int4_ops'),
		),
		uniqueIndex('media_provider_show_id_provider_id_key').using(
			'btree',
			table.showId.asc().nullsLast().op('int4_ops'),
			table.providerId.asc().nullsLast().op('int4_ops'),
		),
...
export const show = pgTable(
	...
	(table) => [
		index('show_first_air_date_idx').using(
			'btree',
			table.firstAirDate.asc().nullsLast().op('date_ops'),
		),
		index('show_language_id_idx').using(
			'btree',
			table.languageId.asc().nullsLast().op('text_ops'),
		),
		index('show_last_air_date_idx').using(
			'btree',
			table.lastAirDate.asc().nullsLast().op('date_ops'),
		),
		index('show_name_idx').using(
			'gin',
			table.name.asc().nullsLast().op('gin_trgm_ops'),
		),
		index('show_vote_average_idx').using(
			'btree',
			table.voteAverage.asc().nullsLast().op('float8_ops'),
		),
		index('show_vote_count_idx').using(
			'btree',
			table.voteCount.asc().nullsLast().op('int4_ops'),
		),
	],
